        ...
        create: [
          {
            ...
            learningSections: {
              create: [
                ...
                { title: "What is a Pure Function?", content: "A pure function is a function that:\nAlways returns the same output for the same input\nHas no side effects (doesn't modify anything outside itself) // Pure function ✅\nfunction isBookAvailable(availableCopies: number): boolean { return availableCopies > 0;\n} // NOT pure ❌ — reads external state\nfunction isBookAvailable(): boolean { return globalBookCount > 0; // depends on outside variable\n} Pure functions are predictable, easy to test, and safe to reuse anywhere.", order: 2 },
                { title: "Why Centralize Logic in a Helper?", content: "Imagine the same availability check scattered across 5 different components: // In Books.tsx\nif (book.availableCopies > 0) { ... } // In BorrowRecords.tsx\nif (book.copies !== 0) { ... } // ← slightly different! // In Dashboard.tsx\nif (book.availableCopies >= 1) { ... } Each variation is a bug waiting to happen. If the rule changes (e.g., \"reserve 1 copy for walk-ins\"), you'd need to update every file. With a centralized helper:\n// utils/helpers.ts\nexport function isBookAvailable(availableCopies: number): boolean { return availableCopies > 0;\n} Now every component imports and uses the same logic. One change = consistent behavior everywhere.", order: 3 },
                { title: "Where to Put Helpers", content: "In React projects, shared utility functions live in a utils/ folder: client/src/\n└── utils/ └── helpers.ts ← shared helper functions go here", order: 4 },
                { title: "Boundary Conditions", content: "When writing availability logic, you need to handle edge cases — inputs at or near the boundary of expected values: | availableCopies | Expected result |\n|-----------------|-----------------|\n| 5 | true (available) |\n| 1 | true (available) |\n| 0 | false (unavailable) |\n| -1 | false (unavailable — defensive) | The 0 boundary is the most important: a book with 0 copies is not available, even though 0 is technically a valid number.", order: 5 },
                { title: "Exporting from a Module", content: "To use your helper in other files, you must export it: // utils/helpers.ts\nexport function isBookAvailable(availableCopies: number): boolean { return availableCopies > 0;\n} And import it where needed:\nimport { isBookAvailable } from '../utils/helpers';", order: 6 },
                { title: "Key Takeaway", content: "Small, focused pure functions are the building blocks of reliable UIs. By centralizing decision logic in a shared helper, you write it once, test it once, and trust it everywhere.", order: 7 },
              ],
            },
            hints: {
              create: [
          ...
          },
          {
            ...
            learningSections: {
              create: [
                ...
                { title: "The Problem: Duplicated Logic", content: "When the same decision appears in multiple components with slight differences, bugs creep in: // Books.tsx\nconst canBorrow = book.availableCopies > 0; // BorrowRecords.tsx\nconst canBorrow = book.copies !== 0; // ← different condition! These two checks look similar but behave differently at edge cases (e.g., negative copies). If a librarian borrows the last copy and copies somehow go negative, one check says \"unavailable\" and the other says \"available.\"", order: 3 },
                { title: "The Fix: Import and Reuse", content: "Replace the inline condition with the shared helper: // Before — inline logic\nconst showBorrowButton = book.availableCopies !== 0; // After — shared helper\nimport { isBookAvailable } from '../utils/helpers';\nconst showBorrowButton = isBookAvailable(book.availableCopies); The behavior is driven by the helper now. If the helper's rule ever changes, all components update automatically.", order: 4 },
        ...
        create: [
          {
            ...
            learningSections: {
              create: [
                ...
                { title: "The Return Flow", content: "When a member returns a book, two things should happen:\nThe BorrowRecord is updated (e.g., returnedAt set, status changed)\nThe Book's availableCopies is incremented by 1 Both changes need to happen — but what if only one of them does?", order: 3 },
                { title: "The Problem: Separate Writes", content: "If these two database updates are made in separate Prisma calls: // Step 1\nawait prisma.borrowRecord.update({ where: { id }, data: { returnedAt: new Date() } }); // Step 2 — what if this crashes or the server restarts here?\nawait prisma.book.update({ where: { id: bookId }, data: { availableCopies: { increment: 1 } } }); ...then a failure between Step 1 and Step 2 leaves the database in a partial state:\nThe record says the book was returned ✅\nBut the inventory never updated ❌ Over time, with repeated partial failures, counts drift and can go negative.", order: 4 },
          ...
          },
          {
            ...
            learningSections: {
              create: [
                ...
                { title: "What is a Transaction?", content: "A database transaction is a group of operations that either all succeed or all fail together. There's no in-between. Think of it like a bank transfer:\nDeduct $100 from Account A\nAdd $100 to Account B If step 2 fails after step 1, the money disappears. A transaction prevents this — it rolls back step 1 if step 2 fails.", order: 2 },
                ...
                { title: "Prisma Transactions", content: "Prisma provides prisma.$transaction() to wrap multiple writes atomically: await prisma.$transaction([ prisma.borrowRecord.update({ where: { id }, data: { returnedAt: new Date() } }), prisma.book.update({ where: { id: bookId }, data: { availableCopies: { increment: 1 } } }),\n]); Both writes succeed together, or neither is committed.", order: 4 },
                { title: "The Concurrency Problem", content: "Even with correct logic, concurrent requests can corrupt data: Timeline (no protection):\nRequest A reads availableCopies = 1\nRequest B reads availableCopies = 1\nRequest A borrows → sets to 0\nRequest B borrows → sets to 0 ← should have been rejected!\nResult: 2 borrows, 0 copies (should be -1 if no guard)", order: 5 },
                { title: "Guard Conditions", content: "A conditional update prevents this by including a safety check in the update itself: prisma.book.updateMany({ where: { id: bookId, availableCopies: { gt: 0 } }, // only update if copies > 0 data: { availableCopies: { decrement: 1 } },\n}) If 0 rows are updated, the borrow is rejected — the book is already gone.", order: 6 },
        ...
        create: [
          {
            ...
            learningSections: {
              create: [
                ...
                { title: "The Feature: Reservation Queue", content: "When all copies of a book are borrowed, a member should be able to join a queue. The queue determines who gets the book next when a copy becomes available. ### Data Model Concept\nReservation { id bookId ← which book memberId ← who reserved it queuePosition ← their place in line (1 = first) status ← RESERVED | READY_FOR_PICKUP | CANCELLED createdAt\n}", order: 3 },
                { title: "Server Side: The API Endpoint", content: "### POST /api/reservations\nCreates a new reservation. Before inserting, the server must validate:\nThe book exists\navailableCopies === 0 (reserving available books makes no sense)\nThe member doesn't already have an active reservation for this book Assigning Queue Position\nQueue position is calculated server-side — count the existing active reservations for the book and add 1:\nconst existingCount = await prisma.reservation.count({ where: { bookId, status: 'RESERVED' }\n});\nconst queuePosition = existingCount + 1; ### GET /api/reservations?bookId=<id>\nReturns the reservation queue for a book, ordered by position. Each row includes member and book details so the UI doesn't need to make extra requests.", order: 4 },
                { title: "Client Side: The Service Layer", content: "In a well-structured React app, API calls live in a service layer — not directly in components: client/src/\n└── services/ └── libraryService.ts ← all API calls live here Your createReservation function in this file calls POST /api/reservations and returns the result.", order: 5 },
                { title: "Client Side: The UI", content: "The Books.tsx page needs conditional rendering:\nIf availableCopies > 0 → show Borrow button\nIf availableCopies === 0 → show Reserve button After a successful reservation, show the member their queue position:\n\"You are #3 in line.\" This number must come from the server response — not calculated on the client.", order: 6 },
          ...
          },
          {
            ...
            learningSections: {
              create: [
                ...
                { title: "What is a Lifecycle?", content: "A lifecycle describes all the states a thing can be in and how it moves between them. For a reservation, the lifecycle looks like this: RESERVED → READY_FOR_PICKUP → (borrowed, reservation complete) ↓\nCANCELLED Each state has meaning:\nRESERVED — in the queue, waiting\nREADY_FOR_PICKUP — a copy is available, this member is next\nCANCELLED — member withdrew from the queue", order: 2 },
                { title: "State Transitions: When and How", content: "### RESERVED → READY_FOR_PICKUP\nThis happens automatically when a book is returned. The return flow should:\nMark the borrow record as returned\nIncrement availableCopies\nCheck if any active reservations exist for this book\nIf yes, promote the first one (lowest queuePosition) to READY_FOR_PICKUP All of this happens inside one transaction — it's one atomic action. ### RESERVED → CANCELLED\nTriggered by the member. The cancellation flow should:\nMark the reservation as CANCELLED\nReindex the remaining active reservations so positions stay continuous ### Reindexing the Queue\nWhen reservation #2 is cancelled, the queue looks like this:\nPosition 1 → Member A (RESERVED)\nPosition 2 → CANCELLED\nPosition 3 → Member C (RESERVED) After reindex:\nPosition 1 → Member A\nPosition 2 → Member C ← was 3, now 2 Reindexing ensures members always see accurate \"You are #N in line\" numbers.", order: 3 },
                { title: "Transactional Queue Mutations", content: "Both promotion and reindexing must happen inside a transaction:\nawait prisma.$transaction(async (tx) => { // cancel the reservation await tx.reservation.update({ where: { id }, data: { status: 'CANCELLED' } }); // reindex remaining const remaining = await tx.reservation.findMany({ where: { bookId, status: 'RESERVED' }, orderBy: { createdAt: 'asc' } }); for (let i = 0; i < remaining.length; i++) { await tx.reservation.update({ where: { id: remaining[i].id }, data: { queuePosition: i + 1 } }); }\n});", order: 4 },
                { title: "Client Side: Reservation List View", content: "The UI should show each reservation with:\nBook title\nCurrent queue position\nStatus (visually distinct for READY_FOR_PICKUP vs RESERVED)\nA cancel button (only for active reservations) Always render based on server data — never compute lifecycle status on the client.", order: 5 },
          ...
          },
          {
            ...
            learningSections: {
              create: [
                ...
                { title: "Step 1: Regression Tests First", content: "Write the test before fixing the code. This proves:\nThe bug exists (test fails before fix)\nYour fix works (test passes after fix)\nThe bug can never silently return (test will catch it) it('should not list returned books as overdue even with stale OVERDUE status', async () => { // create a record that has returnedAt set but status = 'OVERDUE' // call /api/borrow-records/overdue // assert: this record is NOT in the response\n});", order: 3 },
                { title: "Step 2: Centralize the Date Logic", content: "Overdue determination logic (returnedAt === null && dueDate < now) should live in one shared utility: // utils/overdueUtils.ts\nexport function isOverdue(returnedAt: Date | null, dueDate: Date): boolean { return returnedAt === null && dueDate < new Date();\n} This prevents the same logic from being written differently in multiple places (the original cause of the bug).", order: 4 },
                { title: "Step 3: Writing a Postmortem", content: "A postmortem is a short document written after a production incident. It's not about blame — it's about learning and preventing recurrence. A good postmortem includes: ### Symptom\nWhat the user/client observed:\n\"Overdue report shows returned books as overdue\" ### Root Cause\nThe technical reason it happened:\n\"The overdue query filtered by status field which can become stale when the return flow fails mid-way. Books with returnedAt set but status = 'OVERDUE' were incorrectly included.\" ### Fix\nWhat you changed:\n\"Replaced status-based filter with returnedAt IS NULL AND dueDate < NOW() in the Prisma query. Centralized this logic in overdueUtils.ts.\" ### Prevention\nHow to stop it from happening again:\n\"Added regression test coverage. Added code comment warning against using status for overdue classification.\"", order: 5 },
