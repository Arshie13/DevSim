...
{#if isActive}
  ...
  >
    ...
    <span class="card-watermark"
      >{scenario.number.toString().padStart(2, "0")}</span
    >
    ...
    <div class="relative z-[2] flex flex-col h-full px-6 pt-5 pb-5">
      ...
      <div class="flex-shrink-0 flex items-center justify-between mb-3">
        <div
          class="flex items-baseline gap-0.5 font-['Chakra_Petch',monospace] font-bold"
        >
          <span
            class="text-[0.6rem] tracking-widest uppercase opacity-60"
            style="color:{diffColor}">SCN</span
          >
          <span
            class="text-[1.6rem] leading-none tracking-tight ml-1"
        ...
        <div>
          <button on:click={openModal}>Show Previews</button>
        </div>
      </div>
      ...
      <h2
        class="flex-shrink-0 font-['Chakra_Petch',monospace] text-[1.15rem] font-bold text-[#e2e8f0] tracking-wide leading-snug line-clamp-2 mb-2"
      >
        {scenario.title}
      </h2>
          ...
          <div class="epics-list">
            {#each epics as epic}
              <div class="epic-item" style="border-left-color:{diffColor}55;">
                ...
                <span class="epic-levels"
                  >{epic.levelCount} level{epic.levelCount !== 1
                    ? "s"
                    : ""}</span
                >
              </div>
      ...
      <div
        class="flex-shrink-0 flex items-center justify-between pt-3 border-t border-[rgba(7,165,201,0.1)] gap-3"
      >
        <div
          class="flex items-center gap-1.5 text-[0.65rem] font-mono tracking-[0.04em]"
          style="color:{diffColor}99;"
        >
          <Layers class="w-3 h-3" />
          {#if scenario.hasLevels}
            <span
              >{scenario.levelCount} Level{scenario.levelCount !== 1
                ? "s"
                : ""}</span
            >
          {:else}
...
{:else}
  ...
  >
    <div
      class="card-top-bar"
      style="background:{diffColor}; opacity:0.5;"
    ></div>
    <span class="card-watermark"
      >{scenario.number.toString().padStart(2, "0")}</span
    >
    ...
    <div class="relative z-[2] flex flex-col h-full px-6 pt-5 pb-5 flex-1">
      ...
      <h2
        class="font-['Chakra_Petch',monospace] text-[1.15rem] font-bold text-[#e2e8f0] tracking-wide leading-snug line-clamp-2"
      >
        {scenario.title}
      </h2>
    </div>
...
<style>
  ...
  .bracket-tl {
    top: 8px;
    left: 8px;
    border-top: 1.5px solid var(--diff-color);
    border-left: 1.5px solid var(--diff-color);
  }
  .bracket-tr {
    top: 8px;
    right: 8px;
    border-top: 1.5px solid var(--diff-color);
    border-right: 1.5px solid var(--diff-color);
  }
  .bracket-bl {
    bottom: 8px;
    left: 8px;
    border-bottom: 1.5px solid var(--diff-color);
    border-left: 1.5px solid var(--diff-color);
  }
  .bracket-br {
    bottom: 8px;
  ...
  .desc-scroll.expanded::-webkit-scrollbar {
    width: 4px;
  }
