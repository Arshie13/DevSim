<script lang="ts">
  ...
  let paywallStack: ContainerType | null = null;
  ...
  function openPaywall(stack: ContainerType) {
    paywallStack = stack;
    paywallOpen = true;
    restoreError = "";
  }
  ...
  >
    ...
    <div class="space-y-3">
      ...
      <div class="flex items-center gap-3 bg-[#0a0e1a] border border-[rgba(7,165,201,0.12)] rounded-[4px] px-4 py-3">
        <div>
          <p class="font-mono text-[0.85rem] font-semibold text-[#d0d7dd]">{paywallStack.scenarioTitle ?? parseStackName(paywallStack.stacks)}</p>
          {#if paywallStack.scenarioTitle}
            <p class="font-mono text-xs text-[#8892a0]">{parseStackName(paywallStack.stacks)}</p>
          {/if}
