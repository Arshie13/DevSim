...
function buildMernTaskOne(): ITask {
  const taskId = "tutorial-mern-level1-task1";
  ...
  const hints: IHints[] = [
    ...
    },
    {
      ...
      taskId,
      content:
        "Run the seed script from the server workspace so MongoDB is populated before development run.",
    },
  ];
  ...
  const acceptanceCriteria: IAcceptanceCriteria[] = [
    ...
    },
    {
      ...
      order: 2,
      isRequired: true,
      description: "MongoDB seed command runs successfully",
    },
    {
      id: `${taskId}-ac-3`,
      taskId,
...
function buildMernTaskTwo(): ITask {
  const taskId = "tutorial-mern-level1-task2";
...
const MERN_TUTORIAL_DATA: TutorialWorkspaceData = {
  stackKey: "react-express-mongodb",
  stackLabel: "MERN",
  scenarioTitle: "MERN Stack Tutorial",
  scenarioDescription:
    "Set up the MERN tutorial workspace by reading docs, preparing dependencies, seeding MongoDB, and verifying preview.",
  level: 1,
  tasks: [buildMernTaskOne(), buildMernTaskTwo()],
};
...
export function getTutorialWorkspaceData(stackName: string): TutorialWorkspaceData {
  ...
  if (
    ...
    normalized.includes("react-express-mongodb")
  ) {
    return MERN_TUTORIAL_DATA;
  }
...
export const STEPS: TutorialStep[] = [
  ...
  },
  {
    ...
    instruction: 'Move to the server folder to set up backend and MongoDB.',
    hint: 'Run: cd ../server',
    switchTab: 'terminal',
    command: 'cd ../server',
    requireCommand: true,
  ...
  {
    id: 'submit-sprint-preflight',
    title: 'Review Part 1 — Submit Sprint?',
    instruction: 'This is Part 1 of the submit flow. The pre-flight summary shows your completion stats, Sprint Tasks lists what you completed, Workspace Deltas lists the files you changed, and the bottom row shows the XP and Coins you will earn. Click the Next button at the bottom of the modal to continue to Part 2.',
    hint: 'Read through each section, then click Next inside the modal.',
  ...
  },
  {
    ...
    instruction: 'This is Part 2 — Mastery Checkpoint. The modal advances here automatically. Type a short reflection on what you learned (at least 80 characters). Use the Copy button below to paste the example into the field, then click Next at the bottom of the modal.',
    hint: 'Example: "I set up the MERN stack by following the README sequence — first installing root dependencies, then client and server packages. For Task 2, I used the search to quickly locate TodoPage.tsx and made the UI change. This tutorial helped me understand the project structure and practice using the terminal and search features."',
    copyText: 'I set up the MERN stack by following the README sequence — first installing root dependencies, then client and server packages. For Task 2, I used the search to quickly locate TodoPage.tsx and made the UI change. This tutorial helped me understand the project structure and practice using the terminal and search features.',
    target: 'submit-sprint-confirm-button',
    requireTargetClick: true,
    spotlightTarget: 'submit-sprint-modal',
    preferSide: 'bottom',
  },
  {
    ...
    instruction: 'This is Part 3 — Select Layers. The modal advances here automatically. Click Frontend + Database checkboxes.\nNote: In real workspace, this will be checked strictly.',
    hint: '',
