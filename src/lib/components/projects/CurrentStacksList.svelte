...
{#if confirmTarget}
  ...
  >
    <div class="space-y-3">
      <div class="info-row">
        <p class="info-title">{confirmTarget.scenario?.name ?? parseStackName(confirmTarget.containerStacks)}</p>
        {#if confirmTarget.scenario?.name}
          <p class="info-sub">{parseStackName(confirmTarget.containerStacks)}</p>
        {/if}
