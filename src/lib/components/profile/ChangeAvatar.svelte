...
{#if open}
  ...
  >
    ...
    <div
      ...
      class="relative z-10 w-full max-w-xl bg-obsidian-bg-light border border-obsidian-accent/30 rounded-card shadow-[0_0_60px_rgb(var(--accent-rgb)/0.2)] flex flex-col overflow-hidden max-h-[90vh]"
    >
      ...
      <div class="flex items-center justify-between px-5 py-4 border-b border-obsidian-border/50 shrink-0">
        <div>
          <h2 id="select-avatar-title" class="text-sm font-heading font-bold text-obsidian-text-muted tracking-wide">Select Avatar</h2>
          <p class="text-[0.6rem] font-label text-obsidian-text-primary/40 mt-0.5 uppercase tracking-widest">Customise your identity</p>
        </div>
        <div class="flex items-center gap-3">
          <!-- Coin balance -->
          <div class="flex items-center gap-1.5 text-xs font-label text-cyber-warn bg-cyber-warn/10 border border-cyber-warn/25 rounded-chrome px-2.5 py-1.5">
            <Coins class="w-3.5 h-3.5" />
            <span class="tabular-nums">{coins.toLocaleString()}</span>
          </div>
      ...
      <div class="flex shrink-0 border-b border-obsidian-border/50">
        <button
          class="flex-1 py-2.5 text-xs font-label uppercase tracking-widest transition-colors duration-200 border-b-2 {activeTab === 'owned' ? 'border-obsidian-accent text-obsidian-accent' : 'border-transparent text-obsidian-text-primary/40 hover:text-obsidian-text-primary/70'}"
          on:click={() => (activeTab = "owned")}>
          Owned <span class="ml-1 text-[0.55rem] opacity-60 tabular-nums">({ownedList.length})</span>
        </button>
        <button
          class="flex-1 py-2.5 text-xs font-label uppercase tracking-widest transition-colors duration-200 border-b-2 {activeTab === 'premium' ? 'border-cyber-warn text-cyber-warn' : 'border-transparent text-obsidian-text-primary/40 hover:text-obsidian-text-primary/70'}"
          on:click={() => (activeTab = "premium")}>
          Premium <span class="ml-1 text-[0.55rem] opacity-60 tabular-nums">({premiumList.length})</span>
        </button>
        <button
          class="flex-1 py-2.5 text-xs font-label uppercase tracking-widest transition-colors duration-200 border-b-2 {activeTab === 'pass' ? 'border-obsidian-accent text-obsidian-accent' : 'border-transparent text-obsidian-text-primary/40 hover:text-obsidian-text-primary/70'}"
          on:click={() => (activeTab = "pass")}>
          Pass <span class="ml-1 text-[0.55rem] opacity-60 tabular-nums">({passList.length})</span>
        </button>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-5 min-h-0">
            ...
            {#each ownedList as avatar (avatar.id)}
              ...
              <button
                ...
                class="group relative flex flex-col items-center gap-2 p-3 rounded-card border transition-all duration-200
                  {isSelected
                    ? 'border-obsidian-accent bg-obsidian-accent/10 shadow-[0_0_20px_rgb(var(--accent-rgb)/0.3)]'
                    : 'border-obsidian-border/40 bg-obsidian-surface/40 hover:border-obsidian-accent/40 hover:bg-obsidian-accent/5'}"
              >
                <!-- Selected checkmark -->
                {#if isSelected}
                ...
                <div
                  ...
                  style="border-color: {isSelected ? avatar.color : 'rgb(var(--border-rgb) / 0.6)'}; box-shadow: {isSelected ? `0 0 16px ${avatar.color}55` : 'none'};"
                >
                  <img
                    src={avatar.path}
                    alt={avatar.name}
                ...
                <!-- Name -->
                <span class="text-[0.6rem] font-label uppercase tracking-wide text-obsidian-text-primary/60 group-hover:text-obsidian-text-primary transition-colors">
                  {avatar.name}
                </span>

                <!-- Equipped badge -->
                {#if isEquipped}
                  <span class="text-[0.5rem] font-label uppercase tracking-widest text-obsidian-accent bg-obsidian-accent/10 border border-obsidian-accent/30 rounded-chrome px-1.5 py-0.5 whitespace-nowrap">
                    Equipped
                  </span>
                {:else}
        ...
        {:else if activeTab === "premium"}
          ...
          {#if premiumList.length === 0}
            <div class="flex flex-col items-center justify-center h-32 gap-2 text-obsidian-text-primary/30">
              ...
              <p class="text-xs font-label uppercase tracking-wider">You own all premium avatars!</p>
            </div>
          {:else}
            <div class="grid grid-cols-3 sm:grid-cols-4 gap-3">
              {#each premiumList as avatar (avatar.id)}
                ...
                <div
                  class="group relative flex flex-col items-center gap-2 p-3 rounded-card border border-cyber-warn/20 bg-obsidian-surface/40 {canAfford ? 'opacity-100' : 'opacity-70'} cursor-default"
                >
                  <!-- Lock icon -->
                  <div class="absolute top-1.5 right-1.5 text-cyber-warn/60">
                    <Lock class="w-3.5 h-3.5" />
                  </div>

                  <!-- Avatar image with placeholder -->
                  <div
                    class="w-14 h-14 rounded-card flex items-center justify-center overflow-hidden border-2 border-cyber-warn/30 relative"
                  ...
                  >
                    ...
                    >
                      <span class="text-2xl font-heading font-black opacity-30" style="color:{avatar.color};">?</span>
                    </div>
                  </div>

                  <!-- Name -->
                  <span class="text-[0.6rem] font-label uppercase tracking-wide text-cyber-warn/70">
                    {avatar.name}
                  </span>

                  <!-- Price badge -->
                  <div class="flex items-center gap-1 text-[0.6rem] font-label {canAfford ? 'text-cyber-warn' : 'text-obsidian-text-primary/40'} bg-cyber-warn/10 border border-cyber-warn/20 rounded-chrome px-1.5 py-0.5 tabular-nums">
                  ...
                  {#if canAfford}
                    <button
                      ...
                      disabled={isBuying}
                      class="btn-cyber btn-cyber-outline w-full !py-1 !px-1.5 text-[0.55rem]
                        {isBuying ? 'opacity-40 cursor-not-allowed' : ''}"
                    >
                      {isBuying ? "Buying…" : "Buy"}
                    </button>
                  {:else}
                    <span class="text-[0.5rem] font-label uppercase tracking-widest text-cyber-danger/60 whitespace-nowrap">
        ...
        {:else}
          ...
          <p class="mb-3 text-[0.6rem] font-label uppercase tracking-widest text-obsidian-text-primary/40 flex items-center gap-1.5">
          ...
          <div class="grid grid-cols-3 sm:grid-cols-4 gap-3">
            {#each passList as avatar (avatar.id)}
              {@const isEquipped = currentAvatar === avatar.path}
              <div
                class="group relative flex flex-col items-center gap-2 p-3 rounded-card border border-obsidian-accent/25 bg-obsidian-surface/40 cursor-default"
              >
                ...
                <div
                  ...
                  style="border-color: {isEquipped ? avatar.color : 'rgb(var(--accent-rgb) / 0.3)'}; box-shadow: {isEquipped ? `0 0 16px ${avatar.color}55` : 'none'};"
                >
                  <img
                    src={avatar.path}
                    alt={avatar.name}
                ...
                <!-- Name -->
                <span class="text-[0.6rem] font-label uppercase tracking-wide text-obsidian-text-primary/60">
                  {avatar.name}
                </span>
                ...
                {:else}
                  <span class="flex items-center gap-1 text-[0.5rem] font-label uppercase tracking-widest text-obsidian-accent/70 bg-obsidian-accent/10 border border-obsidian-accent/20 rounded-chrome px-1.5 py-0.5 whitespace-nowrap">
      ...
      <div class="shrink-0 px-5 py-3 border-t border-obsidian-border/50 flex items-center justify-end gap-3">
        <button on:click={handleClose} class="btn-cyber btn-cyber-outline !py-2 !px-4 text-xs">
          Cancel
        </button>
        <button
          on:click={handleConfirm}
          disabled={pendingSelection === currentAvatar}
          class="btn-cyber btn-cyber-solid !py-2 !px-4 text-xs disabled:opacity-40 disabled:cursor-not-allowed"
        >
          Apply
        </button>
      </div>
...
{#if confirmAvatar}
  ...
  >
    ...
    <div
      ...
      class="relative z-10 w-full max-w-sm bg-obsidian-bg-light border border-cyber-warn/30 rounded-card shadow-[0_0_60px_rgb(var(--warn-rgb)/0.15)] overflow-hidden"
    >
      ...
      <div class="flex items-start justify-between px-5 pt-5 pb-3">
        <div class="flex items-center gap-2.5">
          <div class="w-8 h-8 rounded-card flex items-center justify-center bg-cyber-warn/15 border border-cyber-warn/30">
            <Coins class="w-4 h-4 text-cyber-warn" />
          </div>
          <div>
            <p id="confirm-purchase-title" class="text-xs font-heading font-bold text-obsidian-text-muted tracking-wide">Confirm Purchase</p>
            <p class="text-[0.6rem] font-label text-obsidian-text-primary/40 uppercase tracking-widest mt-0.5">DevCoins transaction</p>
          </div>
        </div>
        <button
          on:click={closeConfirm}
          class="w-7 h-7 rounded-card bg-obsidian-bg-light hover:bg-cyber-danger/15 border border-obsidian-border/40 hover:border-cyber-danger/40 flex items-center justify-center transition-all duration-200 text-obsidian-text-primary/40 hover:text-cyber-danger"
        >
          <X class="w-3.5 h-3.5" />
        </button>
      </div>
      ...
      <div class="px-5 pb-4 flex flex-col items-center gap-3">
        ...
        >
          ...
          >
            <span class="text-2xl font-heading font-black opacity-30" style="color:{confirmAvatar.color};">?</span>
          </div>
        </div>
        <p class="text-[0.7rem] font-body text-obsidian-text-muted text-center leading-relaxed">
      ...
      <!-- Balance breakdown -->
      <div class="mx-5 mb-4 rounded-card border border-obsidian-border/50 bg-obsidian-surface/40 overflow-hidden text-xs font-label">
        <div class="flex items-center justify-between px-4 py-2.5 border-b border-obsidian-border/30">
          ...
          <span class="flex items-center gap-1 text-cyber-warn tabular-nums">
            <Coins class="w-3 h-3" />
        ...
        <div class="flex items-center justify-between px-4 py-2.5 border-b border-obsidian-border/30">
          ...
          <span class="flex items-center gap-1 text-cyber-danger tabular-nums">
        ...
        <div class="flex items-center justify-between px-4 py-2.5">
          ...
          <span class="flex items-center gap-1 font-bold tabular-nums {(coins - confirmAvatar.price) >= 0 ? 'text-cyber-success' : 'text-cyber-danger'}">
            <Coins class="w-3 h-3" />
      ...
      <div class="px-5 pb-5 flex items-center gap-2">
        <button
          on:click={closeConfirm}
          class="flex-1 btn-cyber btn-cyber-outline !py-2 !px-4 text-xs"
        >
          Cancel
        </button>
        <button
          ...
          class="flex-1 btn-cyber btn-cyber-solid !py-2 !px-4 text-xs disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-1.5"
        >
...
>
  {#if purchasedAvatar}
    <p>
      <span class="font-heading font-bold" style="color:{purchasedAvatar.color};">{purchasedAvatar.name}</span>
