<!--
  FriendsSection.svelte — Friends grid with avatar, name, and level.
-->
<script context="module" lang="ts">
  export interface Friend {
    name: string;
    /** Emoji, SVG path (/avatars/…), or external URL */
    avatar: string;
    level: number;
  }
</script>
          ...
          <div class="relative">
            ...
            >
              {#if isImagePath(friend.avatar)}
                ...
                />
              {:else}
                <span class="text-3xl lg:text-5xl xl:text-6xl 2xl:text-8xl">{friend.avatar}</span>
              {/if}
            </div>
            <!-- Level badge -->
            <div
