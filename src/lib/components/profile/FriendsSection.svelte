...
<section
  class="relative flex-1 min-h-0 bg-obsidian-bg-light border border-obsidian-accent/25 rounded-card overflow-hidden shadow-accent-glow-lg hover:shadow-accent-glow-hover transition-shadow duration-500 flex flex-col"
>
  ...
  <div class="flex-1 p-3 lg:p-4 flex items-center">
    ...
    {:else}
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 lg:gap-4 w-full">
      {#each rivals as rival (rival.id)}
        ...
        >
          ...
          <div class="relative">
            ...
            <!-- Level badge -->
            <div
              class="absolute -bottom-1.5 -right-1.5 min-w-[2rem] h-[2rem] rounded-card bg-gradient-to-br from-cyber-warn to-cyber-warn/70 flex items-center justify-center font-heading text-xs font-bold text-white shadow-[0_0_10px_rgb(var(--warn-rgb)_/_0.5)] border-2 border-obsidian-surface px-1.5"
            >
              {rival.level}
            </div>
          </div>
          ...
          <!-- Name -->
          <span class="relative text-[0.65rem] font-label uppercase tracking-wide text-obsidian-text-primary/70 group-hover:text-obsidian-accent truncate w-full text-center transition-colors duration-300 px-3">
            {rival.username}
          </span>

          <!-- XP -->
          <span class="relative text-[0.6rem] font-label text-obsidian-accent/60 group-hover:text-obsidian-accent/90 transition-colors duration-300">
            {rival.xp} XP
          </span>
