<script lang="ts">
  ...
  import { ShieldCheck, Lock, CreditCard, ArrowLeft, Loader2, GitBranch } from 'lucide-svelte';
  ...
  async function handleSubmit(event: Event) {
    ...
    try {
      const response = await fetch('/api/sandbox/create-payment', {
        method: 'POST',
      ...
      const finalizeResponse = await fetch('/api/sandbox/confirm-purchase', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!finalizeResponse.ok || finalizeData.error) {
        throw new Error(finalizeData.error || 'Failed to finalize purchase. Please contact support.');
      }
...
<div class='min-h-screen bg-obsidian-bg scanlines ambient-glow bg-grid-cyber py-12 px-6'>
    ...
    <div class='grid grid-cols-1 lg:grid-cols-2 gap-8'>
      <div class='bg-obsidian-bg-light/40 border border-obsidian-accent/10 rounded-card p-8 backdrop-blur-md'>
        <div class='flex items-center gap-3 mb-6'>
          <div class='p-2 rounded-lg bg-cyber-cyan/10 border border-cyber-cyan/20'>
            <GitBranch class='w-6 h-6 text-cyber-cyan' />
          </div>
        ...
        <div class='space-y-6'>
          <div class='flex justify-between items-start border-b border-obsidian-accent/10 pb-6'>
            <div>
              ...
              <p class='text-sm font-rajdhani text-obsidian-text-primary/50'>Clone GitHub repos into a workspace</p>
            </div>
          </div>
          ...
          <div class='grid grid-cols-1 gap-4 mt-8'>
            <div class='flex items-start gap-3 p-4 rounded-lg bg-obsidian-bg/40 border border-obsidian-accent/5'>
              <GitBranch class='w-5 h-5 text-cyber-cyan shrink-0' />
              <div>
                <p class='text-xs font-orbitron font-bold text-obsidian-text-muted uppercase tracking-wider mb-1'>One-Time Access</p>
                <p class='text-[0.7rem] font-rajdhani text-obsidian-text-primary/50'>Purchase once, launch sandboxes indefinitely. One active sandbox at a time.</p>
              </div>
            </div>
            <div class='flex items-start gap-3 p-4 rounded-lg bg-obsidian-bg/40 border border-obsidian-accent/5'>
          ...
          <form onsubmit={handleSubmit} class='flex flex-col flex-1'>
            ...
            >
              ...
              {:else}
                ...
                <span class='font-orbitron font-bold uppercase tracking-widest'>Pay {product.price}</span>
              {/if}
            </button>
  ...
  <PurchaseSuccessModal
    ...
    closeLabel='Go to Sandbox'
    onClose={() => goto('/sandbox')}
  >
    <p>
      <span class='font-orbitron font-bold text-cyber-cyan'>Sandbox access</span>
      has been granted to your account!
    </p>
  </PurchaseSuccessModal>
</div>
